<?php use libs\modele\JouerUnMatch;
        ...
        foreach ($matchs as $match) { ?>
            ...
            <section class="section-card">
                ...
                <div class="forms">
                    ...
                    <a href="/gerermatch.php?type=vue&idMatch=<?= htmlspecialchars($match->getIdMatch()) ?>&csrf_token=<?= htmlspecialchars(hash_hmac("sha256", $match->getIdMatch() . $_SESSION['csrf_token'] . "vue", $_SESSION['csrf_token'])) ?>" class="forms saisie button"><p>Voir la feuille de match</p></a>
                    <?php if($match->isValider() || JouerUnMatch::isArchiveFDM($match->getIdMatch())) {
                        ...
                        <a href="/gerermatch.php?type=modification&idMatch=<?= htmlspecialchars($match->getIdMatch()) ?>&csrf_token=<?= htmlspecialchars(hash_hmac("sha256", $match->getIdMatch() . $_SESSION['csrf_token'] . "modification", $_SESSION['csrf_token'])) ?>" class="forms modify button"><p>Modifier le match</p></a>
                    <?php } ?>
                    <form method="post" action="gerermatch.php">
                        ...
                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(hash_hmac("sha256", $match->getIdMatch() . $_SESSION['csrf_token'] . "suppression", $_SESSION['csrf_token'])) ?>">
